...
<details>
<summary> Опишете функционалността на извикванията mq_send(3) и mq_receive(3). Кога тези извиквания могат да завършат с грешка EINTR или EAGAIN?</summary>
<br/>

``int mq_send(mqd_t mqdes, const char *msg_ptr, size_t msg_len, unsigned int msg_prio)`` добавя съобщението към опашката от съобщения, като, ако опашката е пълна, блокира до освобождаване. 
 Съобщенията се нареждат по приоритет. Ако е зададен флаг O_NONBLOCK, при пълна опашка имаме грешка EAGAIN. 

 
``ssize_t mq_receive(mqd_t mqdes, char *msg_ptr, size_t msg_len, unsigned int *msg_prio)`` премахва поредното съобщение от опашката и го записва в буфера. Ако опашката е празна, блокира до получаване на съобщение, а при вдигнат флаг O_NONBLOCK завършва с EAGAIN.

Ако изпълнението на някое от извикванията е прекъснато от сигнал, то завършва с грешка EINTR.
