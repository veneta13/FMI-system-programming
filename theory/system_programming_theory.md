...
- ``int pipe(int pipefd[2])``
	...
	- при успешно създаване на тръба в подаденият като аргумент масив се записват следните файлови дескриптори:
...
- ``int mkfifo(const char *pathname, mode_t mode)``
	- създава се **именувана тръба** - специален вид файл, на указаният като параметър адрес
...
- твърди връзки (**hard link**)
	...
	- примяната на стария файл е същата като промяната на новия, защото те **споделят** един и същи i-node
...
- действие на **сървъра**:
	...
	- ``accept()`` - приема заявка за изграждане на връзка, връща fd на сървъра
...
- ``int sem_init(sem_t *sem, int pshared, unsigned int value)``
	- инициализира **неименувана семафора** на адреса sem с начална стойност value

- ``int sem_post(sem_t *sem)``
	- **увеличава** брояча на (отключва) семафората sem
	- ако броячът стане повече от 0, се събужда друг процес, който заключва семафората

- ``int sem_wait(sem_t *sem)``
	- **намалява** брояча на (заключва) семафората sem
	- ако той стане 0, блокира процеса докато декрементирането не стане възможно

- ``int sem_trywait(sem_t *sem)``
	- **намалява** брояча на (заключва) семафората sem
	- работи само ако в момента **не е** заключена
	- връща **EAGAIN** ако семафората не може да се заключи веднага (т.е. вече е била заключена)

- ``int sem_destroy(sem_t *sem)``
	- унищожава семафората sem
...
- ``ssize_t mq_receive(mqd_t mqdes, char *msg_ptr, size_t msg_len, unsigned int *msg_prio)``
	- **премахва поредното съобщение** от опашката mqdes, и го записва в буфера msg_ptr, и записва в msg_prio приоритета на съобщението
